...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate }) => {
  ...
  const warningEnrollments = ADMIN_STUDENTS.filter((student) => student.status === 'warning').length;
  const expiredEnrollments = ADMIN_STUDENTS.filter((student) => student.status === 'expired').length;
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Witaj — podgląd demo. Oto co dzieje się dziś na platformie.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-green-500 hover:shadow-md transition">
          ...
          <div className="mt-4 flex items-center text-sm">
            <span className="text-slate-500 font-medium">Zapisów ostrzegawczych: {warningEnrollments}</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-brand-accent hover:shadow-md transition">
          ...
          <div className="mt-4 flex items-center text-sm">
            <span className="text-slate-500 font-medium">Wygasłe zapisy: {expiredEnrollments}</span>
          </div>
        </div>
      </div>

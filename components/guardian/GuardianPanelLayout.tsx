...
const sidebarItems: { id: GuardianSectionId; label: string; icon: React.ReactNode }[] = [
...
export const GuardianPanelLayout: React.FC<GuardianPanelLayoutProps> = ({
  ...
  const notifications = [
    { id: 'n1', title: 'Nowy pracownik dodany do firmy', time: '10 minut temu', type: 'users' },
    { id: 'n2', title: 'Pracownik ukończył szkolenie', time: '1 godzina temu', type: 'training' },
    { id: 'n3', title: 'Wygasa uprawnienie w ciągu 14 dni', time: '4 godziny temu', type: 'system' }
  ];
  const unreadCount = 1;
  ...
  const renderSidebarItem = (item: { id: GuardianSectionId; label: string; icon: React.ReactNode }) => {
    const isActive =
      activeSection === item.id ||
      (activeSection === 'course-preview' && item.id === 'courses') ||
      ((activeSection === 'employee-view' || activeSection === 'employee-edit' || activeSection === 'employee-create') && item.id === 'employees');
                      ...
                      >
                        <div
                          className={`shrink-0 h-9 w-9 rounded-full flex items-center justify-center ${
                            notificationIconStyles[item.type] || 'bg-slate-100 text-slate-600'
                          }`}
                        >
                          <Bell size={18} />
                        </div>
